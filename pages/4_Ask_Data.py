...
TOOLS = [
    ...
    {
        "name": "relate_linear_issues",
        "description": (
            "Create a 'related' link between two Linear issues. "
            "Use after creating a theme and its associated deal to link them together."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "issue_id_1": {"type": "string", "description": "First issue identifier (e.g. THE-1234) or UUID"},
                "issue_id_2": {"type": "string", "description": "Second issue identifier (e.g. DEAL-567) or UUID"},
            },
            "required": ["issue_id_1", "issue_id_2"],
        },
    },
    {
        "name": "linear_graphql",
...
8. **p_days reference**: p_days defines the window `WHERE created_at >= CURRENT_DATE - p_days`. Today only=0, today+yesterday=1, last 7 days=6, this week={max(0, (today - week_start).days)}, this month={max(0, (today - month_start).days)}, last 30 days=29. IMPORTANT: The RPC returns rows grouped by `day` — when asked about a specific day (e.g. "yesterday"), you must filter the results by the `day` column. For example, p_days=1 returns BOTH yesterday AND today — sum only the rows where `day` matches the date the user asked about.
...
def _execute_tool(name, args):
    ...
    try:
        ...
        elif name == "relate_linear_issues":

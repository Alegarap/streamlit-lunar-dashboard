...
from lib.charts import COLORS, style_fig, item_detail_viewer
...
if is_all:
    ...
    pills_html = "".join(
        ...
        f'padding:4px 12px; font-family:Fira Sans,sans-serif; font-size:0.75rem; margin:2px 4px;">{d}</span>'
...
def colored_metric(label, value, color):
    st.markdown(
        ...
        f'<p style="font-family:Fira Code,monospace; font-size:2rem; font-weight:600; margin:0; color:{color}; '
    ...
    for rank, cluster in enumerate(matched[:10], 1):
        ...
        with st.expander(f"View items in {label}", expanded=False):
            ...
            if not cluster_items:
                ...
                # Item detail viewer
                item_detail_viewer(cluster_items, key_prefix=f"fy_{cluster['id']}")
